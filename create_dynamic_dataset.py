...
class Storage:
    def __init__(self,config):
        ...
        self.states = np.zeros(shape=(config.num_dynamics_examples,6,5,2),dtype=np.uint8)
...
class DynamicSamples(Dataset):
    ...
    def __getitem__(self,index):
        ...
        action = torch.from_numpy(read_npy_chunk('data/actions.npy',index,1).astype(np.int16))
...
def train_linear(num_workers=2, use_gpu=False):
    trainer = Trainer(backend="torch", num_workers=num_workers, use_gpu=use_gpu,logdir='./ray_results')
    config = Config()
    env = Wordle(word_restriction=config.word_restriction)
...
def train_multi():
    ...
    trainloader = DataLoader(dataset=dataset,batch_size=4096,shuffle= True,num_workers=8)
    data_dict = {
    ...
    mp.spawn(train_network,
        args=(data_dict,config,training_params,),
        nprocs=world_size,
        join=True)
